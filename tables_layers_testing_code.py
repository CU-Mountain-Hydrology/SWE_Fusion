...
# function parameters
# user, date, mmdd date, prev_report_date, model_run, prev_model_Run, masking, bias, bias type
...
date = "20250525"
mmddDate = "0528"
# pillowDate = "25May2025"
prevRepDate = "20250517"
modscagDate = "20250525"

# set model run info
modelRun = "RT_CanAdj_rcn_noSW_woCCR_nofscamsk"
prevModelRun = "ASO_FixLayers_fSCA_RT_CanAdj_rcn_noSW_woCCR_UseThis"
elevationMask = "N"
DIF = "Y"
...
resultsWorkspace = workspaceBase + f"RT_report_data/{date}_results_ET/"

# create directory for model run
if Modmult == "Y":
    RunNameMod = f"fSCA_{modelRun}"
else:
    RunNameMod = modelRun
...
prevRepWorkspace = workspaceBase + f"RT_report_data/{prevRepDate}_results/{prevModelRun}/"
...
meanMask = outWorkspace + f"{mmddDate}_mean_msk.tif"
MODSCAG_tif_plus = f"H:/WestUS_Data/Rittger_data/fsca_v2024.0d/NRT_FSCA_WW_N83/{year}/{modscagDate}.tif"
MODSCAG_tif_plus_proj = outWorkspace + f"fSCA_{modscagDate}_albn83.tif"

# define snow-no snow layer
modscag_0_1 = outWorkspace + f"modscag_0_1_{modscagDate}.tif"
modscag_per = outWorkspace + f"modscag_per_{modscagDate}.tif"
modscag_per_msk = outWorkspace + f"modscag_per_{modscagDate}_msk.tif"

# define snow/no snow null layer
mod_null = outWorkspace + f"modscag_0_1_{date}msk_null.tif"
mod_poly = outWorkspace + f"modscag_0_1_{date}msk_null_poly.shp"
### ASK LEANNE ABOUT UTM
mod_poly_utm = outWorkspace + f"modscag_0_1_{date}_msk_null_poly_utm.shp"

snowPolySel = outWorkspace + f"modscag_{date}_snowline_Sel.shp"
snowPolyElim = outWorkspace + f"modscag_{date}_snowline_Sel_elim.shp"

# define snow pillow gpkg
meanMap = meanWorkspace + f"WW_{mmddDate}_mean_geon83.tif"
meanMap_copy = outWorkspace + f"WW_{mmddDate}_mean_geon83.tif"
meanMap_proj = outWorkspace + f"WW_{mmddDate}_mean_albn83.tif"
meanMapMask = outWorkspace + f"WW_{mmddDate}_mean_msk_albn83.tif"
lastRast = prevRepWorkspace + f"p8_{prevRepDate}_noneg.tif"
DiffRaster = outWorkspace + f"Diff_{date}_{prevRepDate}.tif"

## define rasters
rcn_raw = outWorkspace + f"WW_{date}_phvrcn_mos_noMask.tif"
rcn_glacMask = outWorkspace + f"WW_{date}_phvrcn_mos_masked.tif"
rcn_raw_proj = outWorkspace + f"WW_{date}_phvrcn_albn83.tif"
rcnFinal = outWorkspace + f"phvrcn_{date}_final.tif"
product7 = outWorkspace + f"p7_{date}.tif"
product7_noFsca = outWorkspace + f"p7_{date}_nofsca.tif"
product8 = outWorkspace + f"p8_{date}_noneg.tif"
prod8msk = outWorkspace + f"p8_{date}_noneg_msk.tif"
product9 = outWorkspace + f"p9_{date}.tif"
product10 = outWorkspace + f"p10_{date}.tif"
product11 = outWorkspace + f"p11_{date}.tif"
product12 = outWorkspace + f"p12_{date}.tif"

# output Tables
SWEbandtable = outWorkspace + f"{date}band_swe_table.dbf"
SWEtable = outWorkspace + f"{date}swe_table.dbf"
SWEbandtable100 = outWorkspace + f"{date}swe_table_100.dbf"
SWEbandtable_save = outWorkspace + f"{date}band_swe_table_save.dbf"
SWEtable_save = outWorkspace + f"{date}swe_table_save.dbf"
SWEbandtable100_save = outWorkspace + f"{date}swe_table_100_save.dbf"

# anomoly tables
anombandTable = outWorkspace + f"{date}band_anom_table.dbf"
anomTable = outWorkspace + f"{date}anom_table.dbf"
anomHuc6Table = outWorkspace + f"{date}huc6_anom_table.dbf"
anomHuc6Table_save = outWorkspace + f"{date}huc6_anom_table_save.dbf"
meanTable = outWorkspace + f"{date}mean_table.dbf"
anombandTable_save = outWorkspace + f"{date}band_anom_table_save.dbf"
anomTable_save = outWorkspace + f"{date}anom_table_save.dbf"
meanTable_save = outWorkspace + f"{date}mean_table_save.dbf"

# region tables
anomRegionTable = outWorkspace + f"{date}anomRegion_table.dbf"
anomRegionTable_save = outWorkspace + f"{date}anomRegion_table_save.dbf"

# Modscag 0/1 tables and % tables
scabandtable = outWorkspace + f"{date}band_sca_table.dbf"
scatable = outWorkspace + f"{date}sca_table.dbf"
scabandtable_save = outWorkspace + f"{date}band_sca_table_save.dbf"
scatable_save = outWorkspace + f"{date}_sca_table_save.dbf"
perbandtable = outWorkspace + f"{date}band_per_table.dbf"
pertable = outWorkspace + f"{date}_per_table.dbf"

# create tempoary view for join
SWEbandtableView = outWorkspace + f"{date}band_swe_table_view.dbf"
SWEtableView = outWorkspace + f"{date}swe_table_view.dbf"

# create joined tables
BandtableJoin = outWorkspace + f"{date}band_table.dbf"
WtshdTableJoin = outWorkspace + f"{date}Wtshd_table.dbf"

# Anomaly maps
anomMap = outWorkspace + f"{date}_anom.tif"
anom0_100map = outWorkspace + f"{date}anom0_200.tif"
anom0_100msk = outWorkspace + f"{date}anom0_200_msk.tif"

#SWE maps
SWEzoneMap = outWorkspace + f"{date}_swe_wshd.tif"
SWEHuc6Map = outWorkspace + f"{date}_swe_huc6.tif"
MeanHuc6Map = outWorkspace + f"{date}_mean_huc6.tif"
MeanzoneMap = outWorkspace + f"{date}_mean_wshd.tif"
SWEbandzoneMap = outWorkspace + f"{date}_swe_band_wshd.tif"
MeanBandZoneMap = outWorkspace + f"{date}_mean_band_wshd.tif"
SWEregionMap = outWorkspace + f"{date}_swe_region.tif"
MeanRegionMap = outWorkspace + f"{date}_mean_region.tif"

# mean layer masked for use in creating anomly map
anomMask = outWorkspace + f"{date}_anom_mask.tif"

# statistic
statisticType = "MEAN"

# final output csv tables
WtshdTableJoinCSV = outWorkspace + f"{date}Wtshd_table.csv"
BandtableJoinCSV = outWorkspace + f"{date}band_table.csv"
anomRegionTableCSV = outWorkspace + f"{date}anomRegion_table.csv"
Band100TableCSV = outWorkspace + f"{date}band_table_100.csv"
SCATableJoinCSV = outWorkspace + f"{date}sca_Wtshd_table.csv"
BandSCAtableJoinCSV = outWorkspace + f"{date}sca_band_table.csv"
anomWtshdTableCSV = outWorkspace + f"{date}anomWtshd_table.csv"
anomBandTableCSV = outWorkspace + f"{date}anomBand_table.csv"
anomHUC6TableCSV = outWorkspace + f"{date}anomHUC6_table.csv"
...
if runType == "Normal":
    ...
    for domain in domains:
        ...
        modelTIF = MODWorkspace + f"data/outputs/{modelRun}/{domain}_phvrcn_{date}.tif"

        # extract by mask
        outCut = ExtractByMask(modelTIF, clipFilesWorkspace + f"WW_{domain}_cutline_v2.shp", 'INSIDE')
        outCut.save(cutLinesWorkspace + f"{domain}_{date}_clp.tif")
    ...
    arcpy.MosaicToNewRaster_management(outCutsList, outWorkspace, f"WW_{date}_phvrcn_mos_noMask.tif",
...
# WE DON'T HAVE A PREVIOUS REPORT, NEED TO ADD THIS IN LATER
if DIF == "Y":
