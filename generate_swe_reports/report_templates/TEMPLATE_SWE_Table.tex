...
{% if surveys and headers["% of Average"]|length == 2 %}
\begin{longtable}{@{}p{3.7cm}{% for group, subcols in headers.items() %}{% for _ in subcols %}c{% endfor %}{% endfor %}}
{% else %}
...
{% endif %}
    ...
    {% for row in df.iloc[1:].itertuples(index=False) %}
        ...
        {% if text|length > 30 or (text|length > 23 and surveys and headers["% of Average"]|length == 2) %}
            ...
            \makecell[l]{ {{ text[:split_idx+1] }} \\ {{ text[split_idx+1:] }} }
        {% else %}
            \makecell[l]{ {{ text }} }
        {% endif %}
        {% for val in row[1:] %}
