...
{% set date_groups = ["% of Average", "SWE (in)", "Pillows", "Surveys", "SNODAS*"] %}
...
\begin{longtable}{@{}p{3.5cm}{% for _ in range(flat_cols|length) %}c{% endfor %}}
    \toprule
    ...
    {% for row in df.iloc[1:].itertuples(index=False) %}
        ...
        {% if basin_name != ns.current_basin %}
            ...
            {% if raw_name|length > 22 and (" " in raw_name) %}
    ...
    \specialrule{0.3pt}{4pt}{4pt}
    \multicolumn{ {{ total_cols }} }{l}{\footnotesize*This is a comparison to the SNODAS (SNOw Data Assimilation System) nationwide product from the National Weather Service.} \\
    {% if aso_corrected %}
        \multicolumn{ {{ total_cols }} }{l}{\small§ASO-corrected.} \\
    {% endif %}
\end{longtable}
